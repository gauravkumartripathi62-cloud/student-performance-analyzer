...
async function refreshSignupAvailability() {
  if (!authSignupActionButton || !hasSupabaseClient() || !window.SapasSupabase.checkExistingEmailAccount) {
  ...
  const checkId = ++authSignupCheckSequence;
  try {
    const exists = await window.SapasSupabase.checkExistingEmailAccount(email);
    if (checkId !== authSignupCheckSequence) return;
    authSignupAvailabilityPending = false;
    if (exists) knownRegisteredAuthEmails.add(email);
    authSignupDisabledForExistingAccount = Boolean(exists);
    updateAuthActionState();
  } catch (error) {
    if (checkId !== authSignupCheckSequence) return;
    authSignupAvailabilityPending = false;
...
async function enforceSignupAvailability() {
  if (!hasSupabaseClient() || !window.SapasSupabase.checkExistingEmailAccount) {
    authSignupDisabledForExistingAccount = false;
    authSignupAvailabilityPending = false;
    updateAuthActionState();
    return false;
  ...
  try {
    authSignupAvailabilityPending = true;
    updateAuthActionState();
    const exists = await window.SapasSupabase.checkExistingEmailAccount(email);
    authSignupAvailabilityPending = false;
    if (exists) knownRegisteredAuthEmails.add(email);
    authSignupDisabledForExistingAccount = Boolean(exists);
    updateAuthActionState();
    return authSignupDisabledForExistingAccount;
  } catch (error) {
    authSignupAvailabilityPending = false;
    authSignupDisabledForExistingAccount = false;
    updateAuthActionState();
...
[authEmailInput, authPasswordInput].filter(Boolean).forEach((field) => {
